verilog练习网站HDLBits https://hdlbits.01xz.net/wiki/Main_Page
...
vector array（向量数组）多位信号的集合，对应总线  
声明：类型 [高位：低位] 变量名
使用时注意位宽匹配
{a,b} 拼接运算符，将ab拼接;{3'd5,{2{3'd6}}}可以把3'd6重复两次拼接
可以有[3:-2]这样的负数索引和[0:7]这样反向的和[0:0]这样只有一位的

memory array（存储器数组）
声明： reg [位宽] 变量名 [地址范围]
只能按地址访问单个单元

大端字节序（BE，Big-endian）：最高有效字节放最小内存地址（网络协议常用）
小端序（LE）：最低有效字节放最小内存地址（处理器架构常用）
